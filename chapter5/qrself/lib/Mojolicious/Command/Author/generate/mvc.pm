package Mojolicious::Command::Author::generate::mvc;
...
Mojolicious::Command::Author::generate::mvc - Mojolicious mvc
...
has db => sub {
    <%= $appname %>::DB->new( +{ conf => shift->conf } );
};
...
# add method
# use <%= $appname %>::Model::Example;
# has example => sub {
#     <%= $appname %>::Model::Example->new( +{ conf => shift->conf } );
# };
...
#    $self->helper(
#        model => sub { <%= $appname %>::Model->new( +{ conf => $config } ); } );
#    ...
# }
...
has master => sub { <%= $appname %>::DB::Master->new(); };
...
# use <%= $appname %>::Util qw{now_datetime};
#
# '2017-11-11 13:43:10'
...
sub init {
    ...
    my $t = Test::Mojo->with_roles('+Basic')->new('<%= $appname %>');
    die 'not testing mode' if $t->app->mode ne 'testing';

    # test DB
    # $t->app->commands->run('generate', 'sqlitedb');
    # $t->app->helper(
    #     test_db => sub { <%= $appname %>::DB->new( +{ conf => $t->app->config } ) }
    # );
    return $t;
}
...
my $t = Test::Mojo->with_roles('+Basic')->new('<%= $appname %>')->init;
